...
async def _totaux(
    db: AsyncSession, actifs_seulement: bool, perimetre: Scope
) -> ActionSummaryOut:
    ligne = (
        await db.execute(
            count_all_views(
                ...
                extra_condition=perimetre.actions(),
            )
        )
    ).one()
...
async def portfolio_report(
    ...
    perimetre: Scope = Depends(get_scope),
):
    async def calculer() -> PortfolioReportOut:
        today = today_utc()
        ...
        condition_projets = perimetre.projects()
        if condition_projets is not None:
            stmt_projets = stmt_projets.filter(condition_projets)
        ...
        # Les compteurs par projet ne portent que sur les actions visibles : sinon
        # un collaborateur lirait, dans « son » projet, le volume de travail des
        # autres.
        condition_actions = perimetre.actions()
        if condition_actions is not None:
            stmt_aggr = stmt_aggr.where(condition_actions)
        ...
        return PortfolioReportOut(
            ...
            totals=await _totaux(db, actifs_seulement, perimetre),
            projects=[_projet_vers_rapport(p, lignes.get(p.id)) for p in projets],
        )

    return await get_or_set(
        "reports-portfolio",
        scope_token=perimetre.cache_token,
        params={"scope": scope},
        producer=calculer,
        serialize=lambda valeur: valeur.model_dump_json(),
        deserialize=PortfolioReportOut.model_validate_json,
    )
...
async def project_report(
    ...
    stmt_projet = select(Project).filter(Project.id == project_id)
    condition_projets = perimetre.projects()
    if condition_projets is not None:
        stmt_projet = stmt_projet.filter(condition_projets)
    result = await db.execute(stmt_projet)
    projet = result.scalars().first()
    if not projet:
        # 404 y compris hors périmètre : un 403 confirmerait l'existence du
        # projet, donc du travail confié à quelqu'un d'autre.
    ...
    condition_actions = perimetre.actions()
    if condition_actions is not None:
        stmt = stmt.where(condition_actions)
...
async def workload_report(
    ...
    async def calculer() -> WorkloadReportOut:
        today = today_utc()
        ouverte = and_(Action.status != ActionStatus.TERMINE, Action.progress < 100.0)
        terminee = or_(Action.status == ActionStatus.TERMINE, Action.progress >= 100.0)
        ...
        # Le plan de charge nomme qui porte quoi : c'est précisément ce qu'un
        # compte cloisonné ne doit pas lire sur les autres. Il n'y voit que lui.
        stmt = (
            select(
                Responsable.id,
        ...
        if not perimetre.unrestricted:
            stmt = (
                stmt.where(Responsable.id.in_(perimetre.responsable_ids))
                if perimetre.responsable_ids
                else stmt.where(false())
            )

        lignes = (await db.execute(stmt)).all()

        # Une action que personne ne porte n'est, par définition, dans le
        # périmètre de personne : en publier le nombre à un compte cloisonné
        # révélerait un volume de travail qui ne le regarde pas.
        if perimetre.unrestricted:
            ...
            total_non_assignees = int(non_assignees.scalar_one())
        else:
        ...
        return WorkloadReportOut(
            ...
            unassigned_actions=total_non_assignees,
            responsables=[
                ResponsableReportOut(
    ...
    return await get_or_set(
        "reports-workload",
        scope_token=perimetre.cache_token,
        params={"active_projects_only": active_projects_only},
        producer=calculer,
        serialize=lambda valeur: valeur.model_dump_json(),
        deserialize=WorkloadReportOut.model_validate_json,
    )
...
async def forecast_report(
    ...
    async def calculer() -> ForecastReportOut:
        today = today_utc()
        ...
        stmt_retard = build_actions_query(
            view=ActionView.OVERDUE, active_projects_only=active_projects_only
        )
        if condition is not None:
            stmt_retard = stmt_retard.where(condition)
        retard = await db.execute(
            select(func.count()).select_from(stmt_retard.order_by(None).subquery())
        )
    ...
    return await get_or_set(
        ...
        producer=calculer,
        serialize=lambda valeur: valeur.model_dump_json(),
        deserialize=ForecastReportOut.model_validate_json,
    )
